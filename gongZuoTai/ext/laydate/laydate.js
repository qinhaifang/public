 ...
 * 修正
 * @Author: dwk
 * @Date: 2016年2月1日 修正内容 1.选择日期的时候出现跳月的现象 具体现象 a.选择3月31日的时候
 */
...
!function(win) {
		...
		if (value.replace(/\s/g, '') !== '') {
			if (!exp.test(value)) {
				...
				if (typeof (arr[2]) == "undefined") {
					Dates.elem[as.elemv] = arr[0] + "-" + arr[1] | 0;
					return;
				}
				if (arr[2] < 1) {
					arr[2] = 1;
	...
	Dates.viewDate = function(Y, M, D) {
		...
		var ind = Dates.inymd[2];// 原始的D
		var maxD = Dates.months[M];// 当前月的最大天数

		// 现在的天数超过当前月的最大天数 现在天数等于最大天数
		// 否则现在的天数等于原始的天数 保证是日期框里的的天数本月也是
		// 此处可修改修这个逻辑
		if (D >= maxD) {
			D = maxD;
		} else {
		...
		// 创建日期之前要确保是正确的天数 不要让系统函数自动增加到下一个月
	...
	Dates.view = function(elem, options) {
		...
		if (!Dates.box) {
			...
			div.innerHTML = log.html = '<div class="laydate_top">' + '<div class="laydate_ym laydate_y" id="laydate_YY">' + '<a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a>' + '<input id="laydate_y" readonly><label></label>' + '<a class="laydate_choose laydate_chnext laydate_tab"><cite></cite></a>' + '<div class="laydate_yms">' + '<a class="laydate_tab laydate_chtop"><cite></cite></a>' + '<ul id="laydate_ys"></ul>' + '<a class="laydate_tab laydate_chdown"><cite></cite></a>' + '</div>' + '</div>' + '<div class="laydate_ym laydate_m" id="laydate_MM">' + '<a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a>' + '<input id="laydate_m" readonly><label></label>' + '<a class="laydate_choose laydate_chnext laydate_tab"><cite></cite></a>'
	...
	Dates.events = function() {
		...
		Dates.on(as.oclear, 'click', function() {
			...
			//typeof Dates.options.nowTime === 'function' && Dates.options.nowTime(getDates);
		});
		...
		Dates.on(as.otoday, 'click', function() {
			...
			typeof Dates.options.nowtime === 'function' && Dates.options.nowtime(now);
